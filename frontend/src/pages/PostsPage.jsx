        ...
        <div className="container text-white h-[87vh] mx-auto max-w-screen-sm spacr-y-6 overflow-auto">
          {Allposts.slice()
            ...
            .map((post, index) => (
              <div
                key={index}
                className="bg-gradient-to-r from=blue to-[#3f2182] rounded-lg shadow p-4 space-x-4"
              >
                ...
                <p className="text-white m-3">{post.text}</p>
                ...
                <div className="flex justify-start gap-6 text-white text-sm items-center m-3">
                  <div
                    onClick={() => likePosts(post._id)}
                    className="flex items-center gap-2 cursor-pointer"
                  >
                    <FaThumbsUp className="text-xl hover:text-blue-500" />
                    <span>{post.likes.length} Likes</span>
                  </div>

                  <div className="flex items-center gap-2 cursor-pointer">
                    <FaCommentDots className="text-xl hover:text-green-400" />
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-20 overflow-y-scroll space-y-1">
                    {post.comments.slice(0, 3).map((Comment, index) => (
                      <p
                        key={index}
                        className="text-white rounded flex flex-row justify-start items-start gap-2"
                      >
                        {Comment.text}
                      </p>
                    ))}
                ...
                >
                  ...
                  <div className="flex space-x-2">
                    <button
                      type="button"
                      onClick={(e) => handleSubmit(e, post._id)}
                      className="p-2 text-2xl hover:bg-[#13072e] rounded-full text-white"
